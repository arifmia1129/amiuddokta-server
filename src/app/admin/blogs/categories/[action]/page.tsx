...
function BlogCategoryAction({ params }: any) {
  ...
  const handleGetCategoryInfo = async () => {
    ...
    if (user?.id) {
      const res = await getBlogCategoryById(user?.id);
  ...
  const handleRetrieveData = async () => {
    try {
      setIsLoading(true);
      const { data } = await getBlogCategoryById({ id: id ? id : 0 });
      if (data) {
      ...
      <DefaultLayout>
        <div className="flex h-64 items-center justify-center rounded-lg bg-white p-8 shadow-lg dark:bg-black">
          <div className="text-center">
            ...
            <p className="text-gray-600 mt-4">Loading blog category data...</p>
          </div>
        </div>
      </DefaultLayout>
    );
    ...
    <DefaultLayout>
      <div className="mx-auto max-w-5xl dark:bg-black">
        <div className="mb-4">
          ...
          >
            <ArrowLeft size={16} className="mr-1" /> Back to Blog Categories
          </button>
        </div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-blue-700">
                      Please fill in all required fields for the blog category.
                    </div>
                  </div>
                </div>
              </div>
