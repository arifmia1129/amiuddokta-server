...
import {
  deleteBlogCategory,
  getBlogCategories,
} from "@/app/lib/actions/blogCategory/blogCategory.controller";
import {
  Users,
  PlusCircle,
...
export default function BlogCategoryList() {
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      toast.info("No blog categories selected");
      return;
    }

    confirmAlert({
      title: "Confirm Bulk Deletion",
      message: `Are you sure you want to delete ${selectedIds.length} selected blog category(s)? This action cannot be undone.`,
      buttons: [
        {
          label: "Delete All",
          className: "bg-red-500 text-white px-4 py-2 rounded-md",
          onClick: async () => {
            ...
            try {
              ...
              toast.success(
                `${selectedIds.length} blog categories deleted successfully`,
              );
              setSelectedIds([]);
              setIsAllSelected(false);
              handleGetData();
            } catch (error) {
              toast.error("Failed to delete some blog categories");
            } finally {
              setIsLoading(false);
            }
          },
            ...
            <div className="flex h-64 flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md">
                {searchText || activeFilters.length > 0
                  ...
                  : "There are no blog categories in the system yet. Click 'Add New Category' to create one."}
              </p>
            </div>
          ) : (
                  ...
                  >
                            ...
                            <Image
                              src={
                                category?.featured_image
                                  ? `${constant.baseUrl}/api/files?fileName=${category?.featured_image}`
                                  : "/images/no_image.png"
                              }
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <Link
                          href={`/admin/blogs/categories/edit?id=${category?.id}`}
