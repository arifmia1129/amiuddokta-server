...
import {
  ...
  getPageById,
  updatePage,
} from "@/app/lib/actions/pages/pages.controller";
...
function PagesAction({ params }: any) {
  const [isLoading, setIsLoading] = useState(false);
  const [existingPageInfo, setExistingPageInfo] = useState(null);
  ...
  const handleGetPageInfo = async () => {
    ...
    if (user?.id) {
      const res = await getPageById(user?.id);
      if (res?.success) setExistingPageInfo(res.data);
    }
  };
  ...
  const handleRetrieveData = async () => {
    try {
      setIsLoading(true);
      const { data } = await getPageById({ id: id ? id : 0 });
      if (data) {
        setExistingPageInfo(data);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        router.push("/admin/pages");
        if (id && existingPageInfo) await handleRetrieveData();
      } else {
      ...
      <DefaultLayout>
        <div className="flex h-64 items-center justify-center rounded-lg bg-white p-8 shadow-lg dark:bg-black">
          <div className="text-center">
            ...
            <p className="text-gray-600 mt-4">Loading page data...</p>
          </div>
        </div>
      </DefaultLayout>
    );
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-black">
          <div className="border-b bg-gradient-to-r from-blue-50 to-white p-6 dark:bg-black">
            <h3 className="text-gray-800 flex items-center text-2xl font-bold">
              {isEdit ? (
                <>
                  <User size={24} className="mr-2 text-blue-600" /> Edit Page
                </>
              ) : (
                <>
            ...
            <p className="text-gray-500 mt-1">
              {isEdit ? "Update page information" : "Create a new page"}
            </p>
          </div>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-blue-700">
                      Please fill in all required fields for the page.
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <FormField
                  label="Layout"
                  error={(errors as any).layout?.message}
                  icon={<Info size={16} />}
                >
                  <input
                    {...register("layout")}
                    placeholder="Enter page layout"
                    className="border-gray-300 w-full rounded-md border px-4 py-2.5 transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:bg-black"
                  />
                </FormField>
