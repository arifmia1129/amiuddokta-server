...
import { deletePage, getPages } from "@/app/lib/actions/pages/pages.controller";
import {
  Users,
  PlusCircle,
...
export default function PagesList() {
  ...
  const handleBulkDelete = async () => {
    ...
    confirmAlert({
      ...
      message: `Are you sure you want to delete ${selectedIds.length} selected page(s)? This action cannot be undone.`,
      buttons: [
        {
          label: "Delete All",
          className: "bg-red-500 text-white px-4 py-2 rounded-md",
  ...
  const handleSelectAll = (value: boolean) => {
    ...
    if (checked) {
      const allIds = pageList.map((page: any) => page.id);
  ...
  const handleSelectOne = (pageId: number) => {
    const isExist = selectedIds?.find((id) => id === pageId);
    if (!isExist) {
      setSelectedIds([...selectedIds, pageId]);
    } else {
      setSelectedIds(selectedIds.filter((selectedId) => selectedId !== pageId));
    }
  };
            ...
            <div className="flex h-64 flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md">
                {searchText || activeFilters.length > 0
                  ? `No pages match your current filters. Try different search terms or clear your filters.`
                  : "There are no pages in the system yet. Click 'Add New Page' to create one."}
              </p>
            </div>
          ) : (
                        ...
                        <div className="ml-3 flex items-center">
                          <div className="h-10 w-10 flex-shrink-0">
                            <Image
                              src={
                                page?.featured_image
                                  ? `${constant.baseUrl}/api/files?fileName=${page?.featured_image}`
                                  : "/images/no_image.png"
                              }
                              alt={page?.title || "Page"}
                              width={40}
