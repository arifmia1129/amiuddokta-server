...
export default function ServiceCategoryList() {
  ...
  const handleGetData = async () => {
    ...
    try {
      const res = await getServiceCategories({
        ...
        is_active:
          filterOptions.is_active === "" ? undefined : filterOptions.is_active,
  ...
  useEffect(() => {
    ...
    if (filterOptions.is_active !== "")
      newActiveFilters.push(
        `Status: ${filterOptions.is_active ? "Active" : "Inactive"}`,
      );
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      toast.info("No service categories selected");
      return;
    }

    confirmAlert({
      title: "Confirm Bulk Deletion",
      message: `Are you sure you want to delete ${selectedIds.length} selected service category(s)? This action cannot be undone.`,
      buttons: [
        {
          label: "Delete All",
          className: "bg-red-500 text-white px-4 py-2 rounded-md",
          onClick: async () => {
            ...
            try {
              ...
              toast.success(
                `${selectedIds.length} service categories deleted successfully`,
              );
              setSelectedIds([]);
              setIsAllSelected(false);
              handleGetData();
            } catch (error) {
              toast.error("Failed to delete some service categories");
            } finally {
              setIsLoading(false);
            }
          },
  ...
  const handleSelectAll = (value: boolean) => {
    ...
    if (checked) {
      const allIds = categoryList.map((category: any) => category.id);
  ...
  const handleSelectOne = (categoryId: number) => {
    const isExist = selectedIds?.find((id) => id === categoryId);
    if (!isExist) {
                  ...
                  <div className="border-gray-200 dark:border-gray-700 absolute right-0 z-10 mt-2 w-64 rounded-md border bg-white p-4 shadow-lg dark:bg-black">
                    <div className="mb-4">
                      ...
                      <select
                        ...
                        value={
                          filterOptions.is_active === ""
                            ? ""
                            : filterOptions.is_active.toString()
                        }
                        onChange={(e) =>
            ...
            <div className="flex h-64 flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md">
                {searchText || activeFilters.length > 0
                  ? `No categories match your current filters. Try different search terms or clear your filters.`
                  : "There are no service categories in the system yet. Click 'Add New Category' to create one."}
              </p>
            </div>
          ) : (
              ...
              <tbody className="divide-gray-200 dark:divide-gray-700 divide-y bg-white dark:bg-black">
                {categoryList.map((category: any) => (
                  ...
                  >
                        ...
                        <div className="ml-3 flex items-center">
                          <div className="h-10 w-10 flex-shrink-0">
                            <Image
                              src={
                                category?.icon
                                  ? `${constant.baseUrl}/api/files?fileName=${category?.icon}`
                                  : "/images/no_image.png"
                              }
                    ...
                    <td className=" px-4 py-3.5 max-w-full">
                      <div className="text-gray-900 text-sm dark:text-white max-w-52">
                        {shortenWithLastWord(category?.description, 100)}
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-4 py-3.5">
                      <span
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <Link
                          href={`/admin/services/categories/edit?id=${category?.id}`}
