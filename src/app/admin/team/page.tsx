...
export default function TeamList() {
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      toast.info("No team members selected");
      return;
    }
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            try {
              ...
              toast.success(
                `${selectedIds.length} team members deleted successfully`,
              );
              setSelectedIds([]);
  ...
  const handleSelectAll = (value: boolean) => {
    ...
    if (checked) {
      const allIds = teamList.map((team: any) => team.id);
  ...
  const handleSelectOne = (teamId: number) => {
    const isExist = selectedIds?.find((id) => id === teamId);
    if (!isExist) {
      setSelectedIds([...selectedIds, teamId]);
    } else {
      setSelectedIds(selectedIds.filter((selectedId) => selectedId !== teamId));
    }
  };
        ...
        <div className="mb-6 flex flex-col items-start justify-between gap-4 dark:bg-black sm:flex-row sm:items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-gray-100 text-gray-600 dark:text-gray-300 rounded-full px-2 py-1 text-sm dark:bg-meta-4">
              {total} {total === 1 ? "team member" : "team members"}
            </span>
          </div>
          <Link
            href="/admin/team/add"
            ...
            <div className="flex h-64 flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md">
                {searchText || activeFilters.length > 0
                  ? `No team members match your current filters. Try different search terms or clear your filters.`
                  : "There are no team members in the system yet. Click 'Add New Team Member' to create one."}
              </p>
            </div>
          ) : (
                        ...
                        <div className="ml-3 flex items-center">
                          <div className="h-10 w-10 flex-shrink-0">
                            <Image
                              src={
                                team?.profile_image
                                  ? `${constant.baseUrl}/api/files?fileName=${team?.profile_image}`
                                  : "/images/no_image.png"
                              }
