...
export const GET = async (req: NextRequest) => {
  try {
    ...
    // If ID is provided, retrieve specific media corner item
    if (id) {
      const retrieveResult = await getMediaCornerItemById({ id });
    ...
    // Retrieve list of media corner items
    const mediaParams = {
    ...
    const listResult = await getMediaCornerItems(mediaParams);
