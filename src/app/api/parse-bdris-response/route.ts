...
async function parseHtmlResponse(root: HTMLElement, responseType: string): Promise<ParsedResponse> {
  try {
...
function extractSuccessInfo(root: HTMLElement): ParsedResponse {
  ...
  const redSpans = root.querySelectorAll('span[style*="color:red"], span[style*="color:#red"]');
  
  let applicationId = '';
  
  for (const span of redSpans) {
    const parentText = span.parentNode?.innerText || span.parentNode?.innerHTML || '';
    if (parentText.includes('আবেদনপত্র নম্বর') || parentText.includes('আবেদন নম্বর')) {
  ...
  if (applicationId) {
    // Extract print link
    let printLink = '';
    const printButton = root.querySelector('#appPrintBtn') || 
    ...
    if (printButton) {
      ...
      if (printLink && !printLink.startsWith('http')) {
        printLink = printLink.startsWith('/') ? `https://bdris.gov.bd${printLink}` : `https://bdris.gov.bd/${printLink}`;
      }
    }
...
function extractErrorInfo(root: HTMLElement): ParsedResponse {
  ...
  for (const alertDiv of alertDivs) {
    ...
    if (alertHtml.includes('Error!') || alertHtml.includes('error') || alertHtml.includes('ত্রুটি')) {
      const spans = alertDiv.querySelectorAll('span');
      const errorParts: string[] = [];
      
      for (const span of spans) {
        const spanText = span.innerText.trim();
        if (spanText && !spanText.includes('Error!') && !spanText.includes('×')) {
  ...
  if (!errorMessage) {
    const validationErrors = root.querySelectorAll('.validation-error, .field-error, .form-error, .error-message');
  ...
  if (!errorMessage) {
    ...
    if (bodyText.includes('submission failed') || bodyText.includes('সমস্যা হয়েছে') || 
        bodyText.includes('ব্যর্থ হয়েছে') || bodyText.includes('ত্রুটি')) {
      errorMessage = 'আবেদন জমা দিতে সমস্যা হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।';
    }
  }
...
function extractAdditionalInfo(root: HTMLElement): ParsedResponse['additionalInfo'] {
  ...
  for (const span of greenSpans) {
    const parentText = span.parentNode?.innerText || span.parentNode?.innerHTML || '';
    if (parentText.includes('আবেদনের প্রকৃতি') || parentText.includes('প্রকার')) {
  ...
  for (const span of redSpans) {
    const parentText = span.parentNode?.innerText || span.parentNode?.innerHTML || '';
    if (parentText.includes('তারিখের মধ্যে') || parentText.includes('deadline')) {
...
function extractUsingRegex(html: string): ParsedResponse {
  ...
  const appIdMatch = html.match(/আবেদনপত্র নম্বর\s*:\s*<span[^>]*>([^<]+)<\/span>/);
  ...
  const errorMatch = html.match(/<div[^>]*class="[^"]*alert[^"]*"[^>]*>.*?<strong>\s*Error!?<\/strong>[\s\S]*?<span[^>]*>(.*?)<\/span>/i);
  if (errorMatch && errorMatch[1]) {
    errorMessage = errorMatch[1].trim();
  } else {
    // Method 2: Extract all spans in alert divs
    const spanRegex = /<div[^>]*class="[^"]*alert[^"]*"[^>]*>.*?((<span[^>]*>.*?<\/span>)+).*?<\/div>/is;
    const spanMatch = html.match(spanRegex);
    if (spanMatch && spanMatch[1]) {
      const innerSpans = spanMatch[1].match(/<span[^>]*>(.*?)<\/span>/gis);
      if (innerSpans) {
        const allSpans = innerSpans.map((s: string) => s.replace(/<[^>]+>/g, '').trim());
        errorMessage = allSpans.filter(s => s && s !== 'Error!' && s !== '×').join(' ').trim();
      }
    }
  }
